...
def _get_ticket(ticket_id: uuid.UUID, db: Session) -> Ticket:
...
def get_ticket(ticket_id: uuid.UUID, db: Session = Depends(get_db)) -> Ticket:
    return _get_ticket(ticket_id, db)
...
def assign_ticket(ticket_id: uuid.UUID, payload: TicketAssign, db: Session = Depends(get_db)) -> Ticket:
    ticket = _get_ticket(ticket_id, db)
...
def _apply_transition(ticket_id: uuid.UUID, target: TicketStatus, db: Session) -> Ticket:
    ticket = _get_ticket(ticket_id, db)
...
def accept_ticket(ticket_id: uuid.UUID, db: Session = Depends(get_db)) -> Ticket:
    return _apply_transition(ticket_id, TicketStatus.ACCEPTED, db)


@router.post("/{ticket_id}/start", response_model=TicketRead)
def start_ticket(ticket_id: uuid.UUID, db: Session = Depends(get_db)) -> Ticket:
    return _apply_transition(ticket_id, TicketStatus.IN_PROGRESS, db)


@router.post("/{ticket_id}/complete", response_model=TicketRead)
def complete_ticket(ticket_id: uuid.UUID, db: Session = Depends(get_db)) -> Ticket:
    return _apply_transition(ticket_id, TicketStatus.COMPLETED, db)


@router.post("/{ticket_id}/close", response_model=TicketRead)
def close_ticket(ticket_id: uuid.UUID, db: Session = Depends(get_db)) -> Ticket:
    return _apply_transition(ticket_id, TicketStatus.CLOSED, db)
